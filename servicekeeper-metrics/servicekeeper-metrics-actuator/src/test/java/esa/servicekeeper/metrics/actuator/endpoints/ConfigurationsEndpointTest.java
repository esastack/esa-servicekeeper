...
class ConfigurationsEndpointTest {
    ...
    void testSkConfigs() {
        ...
        final ConfigurationsEndpoint endpoint = new ConfigurationsEndpoint(collector, configs);
        when(collector.configs()).thenReturn(null)
                .thenReturn(Collections.singletonMap(ResourceId.from("A"),
        ...
        then(endpoint.skConfigs()).isEmpty();
        ...
        final Map<String, ServiceKeeperConfigPojo> pojos = endpoint.skConfigs();
        then(pojos.size()).isEqualTo(1);

        final ServiceKeeperConfigPojo pojo = pojos.get("A");
